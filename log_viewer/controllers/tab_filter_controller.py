...
class FilterController(QObject):
    ...
    def cmd_apply_filter(self) -> None:
        ...
        if hasattr(self.tab.main_window, "filter_context_spin"):
            self.tab._filter_context_after = int(self.tab.main_window.filter_context_spin.value())
        ...
        self.tab._filter_worker = FilterWorker(
            self.tab.filter_engine, pattern, use_regex, case, negate, context_after=self.tab._filter_context_after
        )
        ...
        self.tab.register_thread_worker(self.tab._filter_thread, self.tab._filter_worker)
        self.tab._filter_worker.progress.connect(self._on_filter_progress, Qt.ConnectionType.QueuedConnection)
        self.tab._filter_worker.finished.connect(self._on_filter_done, Qt.ConnectionType.QueuedConnection)
        self.tab._filter_worker.finished.connect(self.tab._filter_thread.quit, Qt.ConnectionType.QueuedConnection)
        self.tab._filter_worker.finished.connect(
            self.tab._filter_worker.deleteLater, Qt.ConnectionType.QueuedConnection
        )
        self.tab._filter_thread.finished.connect(
            self.tab._filter_thread.deleteLater, Qt.ConnectionType.QueuedConnection
        )
