...
class EditBuffer:
    ...
    def get(self, line_no: int) -> str | None:
    ...
    def save_to_file(
        ...
        if expected_size is not None and current_stat.st_size != expected_size:
            raise FileChangedError(f"File size changed: expected {expected_size}, got {current_stat.st_size}")
        ...
        if expected_mtime is not None:
            ...
            if abs(current_mtime_ns - expected_mtime_ns) > 1_000_000:
                raise FileChangedError(f"File mtime changed: expected {expected_mtime_ns}, got {current_mtime_ns}")
        ...
        try:
            with (
                ...
                for line_no, raw in enumerate(src):
